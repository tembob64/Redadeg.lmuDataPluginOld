...
namespace Redadeg.lmuDataPlugin
{
    public class MappedBuffer<MappedBufferT>
    {
        ...
        readonly int RF2_BUFFER_VERSION_BLOCK_SIZE_BYTES = Marshal.SizeOf(typeof(LMU_MappedBufferVersionBlock));
        readonly int RF2_BUFFER_VERSION_BLOCK_WITH_SIZE_SIZE_BYTES = Marshal.SizeOf(typeof(LMU_MappedBufferVersionBlockWithSize));
        ...
        public void GetMappedData(ref MappedBufferT mappedData)
        {
            ...
            using (var sharedMemoryStreamView = this.memoryMappedFile.CreateViewStream())
            {
                ...
                var versionHeaderWithSize = new LMU_MappedBufferVersionBlockWithSize();
                var versionHeader = new LMU_MappedBufferVersionBlock();
                ...
                for (retry = 0; retry < MappedBuffer<MappedBufferT>.NUM_MAX_RETRIEES; ++retry)
                {
                    ...
                    if (this.partial)
                    {
                        this.GetHeaderBlock<LMU_MappedBufferVersionBlockWithSize>(sharedMemoryStream, this.RF2_BUFFER_VERSION_BLOCK_WITH_SIZE_SIZE_BYTES, ref versionHeaderWithSize);
                    ...
                    {
                        this.GetHeaderBlock<LMU_MappedBufferVersionBlock>(sharedMemoryStream, this.RF2_BUFFER_VERSION_BLOCK_SIZE_BYTES, ref versionHeader);
                    ...
                    versionHeader = (LMU_MappedBufferVersionBlock)Marshal.PtrToStructure(handleVersionBlock.AddrOfPinnedObject(), typeof(LMU_MappedBufferVersionBlock));
